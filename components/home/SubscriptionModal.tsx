...
export default function SubscriptionModal({ modalVisible, setModalVisible }: ModalProps) {
    ...
    const gotoPlan = () => {
        setModalVisible(false);
        router.push('/homePage/SubscriptionPlan');
    }
            ...
            <View style={styles.modalStyle}>
                <View style={styles.modalBox}>
                    ...
                    <TouchableOpacity style={[styles.btnStyle, { backgroundColor: '#0A5CA8', }]}
                        onPress={gotoPlan}
                    >
                        <Text style={[styles.btnText, { color: 'white' }]}>Let’s subscribe!</Text>
                    </TouchableOpacity>

                    <TouchableOpacity style={[styles.btnStyle, { backgroundColor: '#DAE7F2', }]}
                        onPress={closeModal}
                    >
                        <Text style={[styles.btnText, { color: '#0A5CA8' }]}>Cancel</Text>
                    </TouchableOpacity>
                </View>
            </View>
...
const styles = StyleSheet.create({
    ...
    modalBox: {
        width: wp(86),
        height: Platform.OS === 'ios' ? hp(66) : hp(68),
