...
export default function NotificationReview() {
  ...
  const { data, isFetching } = useHomeNotificationByID(
    isFocused,
    notifId as string
  );
  const { t } = useTranslation();
  const current = getCurrentLanguage();
  const { data: receipt } = useGetEReceipt(
    isFocused,
    data?.meta?.order_id as string
  );
    ...
    <View style={styles.container}>
        ...
        >
            ...
            <TouchableOpacity onPress={() => router.back()}>
              {current === "ar" ? (
                <Image
                  ...
                  resizeMode="contain"
                  style={{ width: wp(6), height: hp(6.2), marginRight: wp(2) }}
                />
              ) : (
                <Image
          ...
          <View style={styles.headerRight}>
            <TouchableOpacity onPress={toggleShowCard}>
              <Image
                source={require("@/assets/icons/bookingMenu.png")}
                resizeMode="contain"
                style={{ width: wp(7.5) }}
              />
            </TouchableOpacity>
      ...
      {isFetching ? (
        ...
        >
          <Barcode
            format="CODE128"
            value={`${receipt?.trm_charge?.transaction_id}`}
            text={`${receipt?.trm_charge?.transaction_id}`}
            width={3}
          ...
          <View style={[styles.summarCard, { marginTop: hp(2) }]}>
            ...
            >
              ...
              <Text style={styles.summaryValue}>
                {receipt?.trm_order?.service?.service?.title}
              </Text>
            </View>
            <View
              style={[
            ...
            >
              ...
              <Text style={styles.summaryValue}>
                {receipt?.trm_order?.service?.service?.sub_title}
              </Text>
            </View>
            {receipt?.trm_customer_subscriptions?.subscription && (
              <View
                style={[
                  styles.summarRow,
                  {
            ...
            >
              ...
              <Text style={styles.summaryValue}>
                {formatDate(receipt?.trm_charge?.date)} |{" "}
                {formatTime(receipt?.trm_charge?.date)}
              </Text>
            </View>
            <View
              style={[
          ...
          <View style={[styles.summarCard, { marginTop: hp(2) }]}>
            ...
            >
              ...
              <Text style={styles.summaryValue}>
                {t("AED")} {formatNumber(receipt?.trm_charge?.amount / 100)}.00
              </Text>
            </View>
            {receipt?.trm_order?.discounted_amount && (
              <View
                style={[
                  styles.summarRow,
                  {
              ...
              >
                ...
                <Text style={[styles.summaryValue, { color: "#0a5ca8" }]}>
                  - {t("AED")}{" "}
                  {formatNumber(receipt?.trm_order?.discounted_amount)}.00
                </Text>
              </View>
            )}
            ...
            >
              ...
              <Text style={[styles.summaryValue]}>
                {receipt?.trm_charge?.payment_method}
              </Text>
            </View>
            ...
            >
              ...
              >
                <Text style={[styles.summaryValue]}>
                  {receipt?.trm_charge?.transaction_id}
                </Text>
                <TouchableOpacity>
                  <Ionicons
                    name="copy-outline"
